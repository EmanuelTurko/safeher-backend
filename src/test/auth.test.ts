...
describe("Authentication API", () => {
  ...
  it("should register a new user", async () => {
    const response = await request(app).post("/api/auth/register").send({ email: USER_EMAIL, fullName: TEST_fullName, password: USER_PASSWORD });

    expect(response.status).toBe(201);
  });

  it("should not register an existing user", async () => {
    const response = await request(app).post("/api/auth/register").send({ email: USER_EMAIL, fullName: "existinguser", password: "password123" });
  ...
  it("should log in with Google", async () => {
    const response = await request(app).post("/api/auth/login-with-google").send({ email: "googleuser@example.com", fullName: "googleuser" });
