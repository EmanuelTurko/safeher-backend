...
describe("User API", () => {
  ...
  beforeAll(async () => {
    ...
    // Register a new user
    const registerResponse = await request(app).post("/api/auth/register").send({ email: USER_EMAIL, fullName: TEST_fullName, password: USER_PASSWORD });
  ...
  describe("Update User Profile", () => {
    ...
    const newfullName = "updatedfullName";
    ...
    it("should update user profile with valid token and userId", async () => {
      ...
      expect(response.body.user).toHaveProperty("fullName", newfullName);
    });
